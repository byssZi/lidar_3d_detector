...
):stream_(stream)
{
  ...
  if (!trtCache.is_open())
  {
    ...
    networkConfig->addOptimizationProfile(profile);
      ...
      auto serializedModel = builder->buildSerializedNetwork(*network, *networkConfig);
      if (!serializedModel)
      {
          std::cerr << ": Serialized model creation failed!" << std::endl;
          exit(-1);
      }
      ...
      trtOut.write((char*)serializedModel->data(), serializedModel->size());
      trtOut.close();
...
#if NV_TENSORRT_MAJOR >= 10
  nvinfer1::Dims TRT::get_binding_shape(const char* tensorName)
  {
    return context->getTensorShape(tensorName);
  }
#else
...
):stream_(stream)
{
  ...
  //output of TRT
  #if NV_TENSORRT_MAJOR >= 10
    const char* tensorName = "output_boxes";  // Replace with the actual input tensor name
    box_size = (trt_->get_binding_shape(tensorName).d[1]) * 9 * sizeof(float);
  #else
